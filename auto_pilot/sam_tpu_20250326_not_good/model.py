...
class Model:
    ...
    my_model = "/home/pi/autopilot/autopilot/models/sam_tpu/merged_model_120x160_12heads.tflite"
    ...
    def predict(self, image):
        ...
        pred_speed = int(np.mean(np.squeeze(all_outputs[-3:])[:,0]) > 0.33)

        pred_speed_s = np.squeeze(all_outputs[-3:])[:,0]
        ...
        # if (arrow_turn_right_raw > 0.3 or arrow_turn_left_raw > 0.3) and arrow_left_raw > 0.5:
        #     pred_angle = 0
        ...
        speed = np.around(pred_speed).astype(int) * 100
